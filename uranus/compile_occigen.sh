...
cd ICOSA_LMDZ
./make_icosa_lmdz -p std -p_opt "-b 20x25 -s 3" -parallel mpi_omp -arch X64_OCCIGEN -arch_path ../ARCH -full -job 8 > $here/log_compile 2>&1
...
cd $here
cp -rf ../trunk/ICOSA_LMDZ/bin/icosa_lmdz.exe ./
cp -rf ../trunk/icosa_lmdz.version ./
